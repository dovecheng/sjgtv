...
import 'package:sjgtv/src/source/sources_api_provider.dart';
...
import 'package:sjgtv/src/source/add_source_page.dart';
...
class _SourceManagePageState extends ConsumerState<SourceManagePage>
  ...
  Future<void> _toggleSource(SourceModel source) async {
    try {
      final ApiResultModel<SourceModel> result =
          await _apiService.toggleSource(source.uuid);
      if (result.isSuccess) {
  ...
  Future<void> _deleteSource(SourceModel source) async {
    ...
    try {
      final ApiResultModel<Object> result =
          await _apiService.deleteSource(source.uuid);
      if (!mounted) return;
      if (result.isSuccess) {
        ref.invalidate(sourcesProvider);
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final AsyncValue<List<SourceModel>> sourcesAsync = ref.watch(sourcesProvider);
    return Scaffold(
