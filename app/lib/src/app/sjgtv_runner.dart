...
import 'package:sjgtv/src/source/source_count_provider.dart';
import 'package:sjgtv/src/proxy/proxy_count_provider.dart';
import 'package:sjgtv/src/tag/tag_count_provider.dart';
...
import 'package:sjgtv/src/app/json_adapter_provider.dart';
...
class _ConfigLoader {
  Future<void> loadInitialConfig() async {
    ...
    if (await $ref.read(sourceCountStorageProvider.future) == 0) {
      await _initializeSourceModels(uuid);
    } else {
      log.d(() => 'sources 已有数据，跳过初始化');
    }

    if (await $ref.read(proxyCountStorageProvider.future) == 0) {
      await _initializeProxies(uuid);
    } else {
      log.d(() => 'proxies 已有数据，跳过初始化');
    }

    if (await $ref.read(tagCountStorageProvider.future) == 0) {
      await _initializeTagModels(uuid);
    } else {
      log.d(() => 'tags 已有数据，跳过初始化');
    }
  }

  Future<void> _initializeSourceModels(Uuid uuid) async {
    try {
      final Dio dio = Dio();
      ...
      if (response.statusCode == 200) {
        ...
        for (final dynamic source in config['sources']) {
          try {
            ...
            await $ref.read(sourcesStorageProvider.notifier).addSource(newSourceModel);
            savedCount++;
  ...
  Future<void> _initializeProxies(Uuid uuid) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await $ref.read(proxiesStorageProvider.notifier).addProxyModel(newProxyModel);
        log.d(() => '成功保存代理配置');
      } else {
  ...
  Future<void> _initializeTagModels(Uuid uuid) async {
    try {
      final Dio dio = Dio();
      log.d(() => '开始加载 tags 配置...');
      final Response<dynamic> response = await dio.get<dynamic>(
