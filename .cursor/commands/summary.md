...
- **更新**：文件已存在则：
  ...
  5. **历史**：在文件末尾用 `## 历史`，下加 `### YYYY-MM-DD HH:mm` 小节，再列本次「变更摘要」或「完成项」；若有从待办中移除或标记已取消的条目，也一并记入（如「已取消：某某」），最后写回文件。**历史小节标题必须包含日期与时间**（格式 `### YYYY-MM-DD HH:mm`），新增时使用**当前**日期时间；同一天多次更新时可酌情在时间后加简短括号说明，如 `### 2026-01-30 14:32 (补充计划)`，但不得省略 `HH:mm`。
