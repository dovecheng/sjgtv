...
变更后收尾命令：按顺序执行「检查修复 → 优化 → 摘要 → 提交推送」，并给出下一步建议。用户说「收尾」「ok」或执行 `/ok` 时使用。可 **@ 摘要文件**（如 `/ok @.cursor/summaries/xxx.md`）；若指定了摘要，步骤 3、5 均以该文件为准。

**执行逻辑**：先检查是否有文件修改（如 `git status`）。无变更则直接结束；有变更则按下述顺序执行，不跳步、不颠倒。
