...
extension ResponsiveLayoutExtension on BuildContext {
  ...
  /// 判断是否应该启用焦点系统（TV 启用焦点）
  bool get shouldEnableFocus => isTV;

  /// 判断是否应该显示状态栏（TV 隐藏状态栏）
  bool get shouldShowStatusBar => !isTV;

  /// 判断是否应该显示系统导航栏（TV 隐藏系统导航栏）
  ...
  double get iconScale {
    ...
    if (width >= LayoutBreakpoints.tv) {
      ...
      return 1.15; // 平板: 图标放大 15%
    } else {
      return 1.0; // 手机: 正常大小
    }
  }

  /// 判断是否应该使用较大的触摸目标（TV 和平板需要更大的触摸目标）
  bool get useLargeTouchTarget => isTV || isTablet;
  ...
  /// 判断是否应该使用宽松布局（TV 和平板）
  bool get isSpaciousLayout => isTV || isTablet;
